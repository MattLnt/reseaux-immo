...
export default async function DashboardAgencePage() {
  ...
  const [catalogueBiens, catalogueAgences] = await Promise.all([
    ...
    prisma.agence.count({
      where: { 
        id: { not: session.user.agenceId },
        biens: { some: { statut: 'ACTIF' } }
      }
    })
  ]);
  ...
  return (
    <div style={{ maxWidth: "100%", margin: "-36px -40px" }}>
      ...
      <div style={{ padding: "0 40px 40px" }}>
        ...
        {/* Actions rapides */}
        <div style={{ background: '#FFFFFF', borderRadius: 16, border: '1px solid #E8EDF2', padding: 24, marginBottom: 24 }}>
          <h2 style={{ fontSize: 16, fontWeight: 700, color: '#002B54', margin: '0 0 16px' }}>
            Actions rapides
          </h2>
          <div className="dashboard-actions" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            <ActionCard
              href="/dashboard/agence/mes-biens/nouveau"
              icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>}
              title="Ajouter un bien"
              subtitle="Nouveau mandat"
              primary
            />
            <ActionCard
              ...
              title="Catalogue réseau"
              subtitle={`${catalogueBiens} biens disponibles`}
            />
            <ActionCard
        ...
        <div className="dashboard-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 24 }}>
          ...
          <DashboardSection title="Le réseau">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <StatBox 
                value={catalogueAgences} 
                label="Agences partenaires" 
                color="blue" 
              />
              <StatBox 
                value={catalogueBiens} 
