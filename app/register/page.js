...
export default function RegisterAgencePage() {
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#002B54', padding: 20, position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="register-grid" style={{ width: '100%', maxWidth: 1180, display: 'grid', gridTemplateColumns: '500px 1fr', gap: 40, alignItems: 'center', position: 'relative', zIndex: 1, padding: '40px 0' }}>
        ...
        <div style={{ background: '#FFFFFF', borderRadius: 20, padding: '32px 28px', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div>
              <div style={sectionLabelStyle}>Votre agence</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Numéro IPI</label>
                    <div style={{ position: 'relative' }}>
                      <div style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2"><rect x="3" y="4" width="18" height="16" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/><line x1="7" y1="15" x2="11" y2="15"/></svg>
                      </div>
                      <input type="text" name="numeroIPI" value={form.numeroIPI} onChange={handleChange} required placeholder="Ex: 506.123" style={inputStyle}
                        onFocus={e => e.target.style.borderColor = '#FF9500'} onBlur={e => e.target.style.borderColor = '#E8EDF2'} />
                    </div>
                  </div>
                </div>
            ...
            <div>
              <div style={sectionLabelStyle}>Contact référent</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Prénom</label>
                    <div style={{ position: 'relative' }}>
                      <div style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                      </div>
                      <input type="text" name="prenomContact" value={form.prenomContact} onChange={handleChange} required placeholder="Jean" style={inputStyle}
                        onFocus={e => e.target.style.borderColor = '#FF9500'} onBlur={e => e.target.style.borderColor = '#E8EDF2'} />
                    </div>
                  </div>

                  <div>
                    <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Nom</label>
                    <div style={{ position: 'relative' }}>
                      <div style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                      </div>
                      <input type="text" name="nomContact" value={form.nomContact} onChange={handleChange} required placeholder="Dupont" style={inputStyle}
                        onFocus={e => e.target.style.borderColor = '#FF9500'} onBlur={e => e.target.style.borderColor = '#E8EDF2'} />
                    </div>
                  </div>
                </div>
            ...
            <button type="submit" disabled={!allValid || loading}
              style={{ width: '100%', background: !allValid || loading ? '#E5E7EB' : '#FF9500', color: '#FFFFFF', fontWeight: 700, padding: 12, borderRadius: 8, border: 'none', cursor: !allValid || loading ? 'not-allowed' : 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 4 }}>
              {loading
        ...
        <div className="register-hero">
          ...
          <h2 style={{ fontSize: 32, fontWeight: 700, color: '#FFFFFF', lineHeight: 1.2, margin: '0 0 16px', letterSpacing: '-0.01em' }}>
            Le réseau qui connecte<br />les agences immobilières
          </h2>
          <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.7)', lineHeight: 1.6, margin: '0 0 32px', maxWidth: 480 }}>
            Partagez vos mandats avec d'autres agences et multipliez vos opportunités de vente.
          </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              ...
              { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>, title: 'Recevez des acheteurs', desc: 'qualifiés du réseau' },
              { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>, title: 'Offre de lancement', desc: '150€/mois HTVA' },
            ].map((item, i) => (
      ...
      <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 16, position: 'relative', zIndex: 1 }}>
        © 2026 OnShare
      </p>
    </main>
  )
