...
const CONTACT_EMAIL = "contact@onshare.be";
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr>
          <td style="background:#001B38;border-radius:16px 16px 0 0;padding:32px 40px;">
            <table width="100%" cellpadding="0" cellspacing="0"><tr>
              <td><span style="font-size:22px;font-weight:700;color:#ffffff;letter-spacing:-0.02em;">On<span style="color:#FF9500;">Share</span></span></td>
        ...
        <tr>
          <td style="background:#002B54;padding:48px 40px 40px;">
            ...
            <p style="color:#9CA3AF;font-size:15px;line-height:1.7;margin:0;">Bonjour <strong style="color:#ffffff;">${nomAgence}</strong>, nous avons bien reçu votre demande d'inscription sur la plateforme OnShare. Votre compte est actuellement en cours d'examen par notre équipe.</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#001B38;border-radius:0 0 16px 16px;padding:24px 40px;">
            <table width="100%" cellpadding="0" cellspacing="0"><tr>
              <td><p style="color:#6B7280;font-size:12px;margin:0;">© 2026 OnShare</p></td>
              <td align="right"><a href="mailto:${CONTACT_EMAIL}" style="color:#FF9500;font-size:12px;text-decoration:none;">${CONTACT_EMAIL}</a></td>
            </tr></table>
          </td>
        </tr>
...
export async function sendAgenceValidee({ to, nomAgence }) {
  await sendEmail({
    to,
    subject: "Votre compte OnShare est activé",
    html: `
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr>
          <td style="background:#002B54;padding:48px 40px 40px;">
            ...
            <p style="color:#9CA3AF;font-size:15px;line-height:1.7;margin:0 0 32px;">Bonjour <strong style="color:#ffffff;">${nomAgence}</strong>, votre accès à la plateforme OnShare est désormais ouvert. Vous pouvez vous connecter et commencer à utiliser toutes les fonctionnalités.</p>
...
export async function sendAgenceDesactivee({ to, nomAgence }) {
  await sendEmail({
    to,
    subject: "Votre compte OnShare a été désactivé",
    html: `
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr>
          <td style="background:#002B54;padding:48px 40px 40px;">
            ...
            <p style="color:#9CA3AF;font-size:15px;line-height:1.7;margin:0;">Bonjour <strong style="color:#ffffff;">${nomAgence}</strong>, nous vous informons que votre compte OnShare a été désactivé. Vous n'avez plus accès à la plateforme.</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#ffffff;padding:40px;">
            <div style="background:#FAFDFD;border-radius:12px;padding:24px;border:1px solid #E8EDF2;margin-bottom:24px;">
              ...
              <p style="font-size:13px;color:#5A6B7D;margin:0;line-height:1.6;">Notre équipe peut vérifier votre dossier et réactiver votre compte si nécessaire. Écrivez-nous à <a href="mailto:${CONTACT_EMAIL}" style="color:#FF9500;text-decoration:none;font-weight:600;">${CONTACT_EMAIL}</a>.</p>
            </div>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr>
          <td style="background:#001B38;border-radius:0 0 16px 16px;padding:24px 40px;">
            <table width="100%" cellpadding="0" cellspacing="0"><tr>
              <td><p style="color:#6B7280;font-size:12px;margin:0;">Notification administrative — OnShare</p></td>
              <td align="right"><a href="mailto:${CONTACT_EMAIL}" style="color:#FF9500;font-size:12px;text-decoration:none;">${CONTACT_EMAIL}</a></td>
            </tr></table>
          </td>
        </tr>
...
// ─────────────────────────────────────────
// 5. EMAIL — Contact : nouveau message depuis la landing
// ─────────────────────────────────────────
export async function sendContactMessage({ nom, nomAgence, email, telephone, message }) {
  // Sanitize message pour HTML (échapper les < >)
  const safeMessage = (message || "").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, "<br>");

  await sendEmail({
    to: CONTACT_EMAIL,
    replyTo: email,
    subject: `Nouveau message — ${nomAgence || nom}`,
    html: `
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr>
          <td style="background:#001B38;border-radius:16px 16px 0 0;padding:32px 40px;">
            <table width="100%" cellpadding="0" cellspacing="0"><tr>
              <td><span style="font-size:22px;font-weight:700;color:#ffffff;letter-spacing:-0.02em;">On<span style="color:#FF9500;">Share</span></span></td>
              <td align="right"><span style="background:rgba(255,149,0,0.15);color:#FF9500;font-size:11px;font-weight:600;padding:5px 12px;border-radius:20px;letter-spacing:0.05em;">NOUVEAU MESSAGE</span></td>
            </tr></table>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#002B54;padding:48px 40px 40px;">
            <div style="width:56px;height:56px;background:rgba(255,149,0,0.15);border-radius:16px;text-align:center;line-height:56px;font-size:24px;margin-bottom:20px;">✉️</div>
            <h1 style="color:#ffffff;font-size:26px;font-weight:700;margin:0 0 12px;letter-spacing:-0.02em;line-height:1.2;">Nouveau message depuis<br><span style="color:#FF9500;">le formulaire de contact</span></h1>
            <p style="color:#9CA3AF;font-size:15px;line-height:1.7;margin:0;">Un visiteur vient de remplir le formulaire de contact sur onshare.be. Vous pouvez lui répondre directement en cliquant "Répondre" — son email est en reply-to.</p>
          </td>
        </tr>

        <tr>
          <td style="background:#ffffff;padding:40px;">
            <p style="font-size:11px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.08em;margin:0 0 20px;">COORDONNÉES</p>
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #F0F3F7;border-radius:12px;overflow:hidden;margin-bottom:28px;">
              <tr style="background:#FAFDFD;">
                <td style="padding:14px 20px;color:#5A6B7D;font-size:13px;border-bottom:1px solid #F0F3F7;width:140px;">Nom</td>
                <td style="padding:14px 20px;color:#002B54;font-weight:700;font-size:14px;text-align:right;border-bottom:1px solid #F0F3F7;">${nom || "—"}</td>
              </tr>
              <tr>
                <td style="padding:14px 20px;color:#5A6B7D;font-size:13px;border-bottom:1px solid #F0F3F7;">Agence</td>
                <td style="padding:14px 20px;color:#002B54;font-weight:700;font-size:14px;text-align:right;border-bottom:1px solid #F0F3F7;">${nomAgence || "—"}</td>
              </tr>
              <tr style="background:#FAFDFD;">
                <td style="padding:14px 20px;color:#5A6B7D;font-size:13px;border-bottom:1px solid #F0F3F7;">Email</td>
                <td style="padding:14px 20px;color:#FF9500;font-weight:700;font-size:13px;text-align:right;border-bottom:1px solid #F0F3F7;"><a href="mailto:${email}" style="color:#FF9500;text-decoration:none;">${email}</a></td>
              </tr>
              <tr>
                <td style="padding:14px 20px;color:#5A6B7D;font-size:13px;">Téléphone</td>
                <td style="padding:14px 20px;color:#002B54;font-weight:700;font-size:13px;text-align:right;">${telephone || "—"}</td>
              </tr>
            </table>

            <p style="font-size:11px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:0.08em;margin:0 0 12px;">MESSAGE</p>
            <div style="background:#FAFDFD;border:1px solid #F0F3F7;border-radius:12px;padding:20px 24px;border-left:3px solid #FF9500;">
              <p style="font-size:14px;color:#002B54;margin:0;line-height:1.7;">${safeMessage}</p>
            </div>

            <div style="margin-top:28px;text-align:center;">
              <a href="mailto:${email}" style="display:inline-block;background:#FF9500;color:#ffffff;padding:14px 28px;border-radius:10px;text-decoration:none;font-weight:700;font-size:14px;">Répondre directement →</a>
            </div>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#001B38;border-radius:0 0 16px 16px;padding:24px 40px;">
            <table width="100%" cellpadding="0" cellspacing="0"><tr>
              <td><p style="color:#6B7280;font-size:12px;margin:0;">Notification automatique — OnShare</p></td>
              <td align="right"><a href="mailto:${CONTACT_EMAIL}" style="color:#FF9500;font-size:12px;text-decoration:none;">${CONTACT_EMAIL}</a></td>
            </tr></table>
          </td>
        </tr>
